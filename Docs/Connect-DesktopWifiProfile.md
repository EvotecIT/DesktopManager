...
## DESCRIPTION
The command waits for a Windows WLAN Auto Configuration completion notification. Cancelling or timing out stops the wait but does not cancel an attempt already accepted by Windows.
...
### -Timeout
How long to wait for a Windows connection completion notification.
