...
### __AllParameterSets
```powershell
Send-DesktopControlKey [-Control] <WindowControlInfo> [-Keys] <VirtualKey[]> [-EnsureForeground] [-AllowForegroundInput] [-Verify] [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

## DESCRIPTION
Sends virtual key presses directly to a window control.

## EXAMPLES

### EXAMPLE 1
```powershell
Send-DesktopControlKey -Control $ctrl -Keys @([DesktopManager.VirtualKey]::VK_A)
```
...
### -Control
Control that will receive the keys.
...
### -EnsureForeground
Bring the parent window to the foreground before UI Automation key fallback.
...
### -Verify
Re-query the control after sending keys and report the observed postcondition.
