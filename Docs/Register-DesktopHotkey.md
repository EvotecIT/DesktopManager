...
### __AllParameterSets
```powershell
Register-DesktopHotkey -Key <VirtualKey> -Action <scriptblock> [-Modifiers <HotkeyModifiers>] [-PassThru] [-WhatIf] [-Confirm] [<CommonParameters>]
```

## DESCRIPTION
Registers a global desktop hotkey.

Registers a system wide hotkey and runs the provided script block when triggered.

## EXAMPLES

### EXAMPLE 1
```powershell
Register-DesktopHotkey -Key F12 -Modifiers Control,Alt -Action { "Hotkey" }
```
...
### -Action
Script block executed when the hotkey fires.
...
Parameter Sets: __AllParameterSets
Aliases: None
Possible values: None, Alt, Control, Shift, Win, NoRepeat
